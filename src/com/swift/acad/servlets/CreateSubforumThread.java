 ...
 */
@WebServlet("/CreateSubforumThread")
public class CreateSubforumThread extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CreateSubforumThread() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int subforumId = Integer.parseInt(request.getParameter("subforum_id"));
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into threads (thread_name, created_at, subforum_id, user_id) values (?, ?, ?, ?)");

			ps.setString(1, thread);
			ps.setDate(2, new java.sql.Date(System.currentTimeMillis()));
			ps.setInt(3, subforumId);
			ps.setInt(4, id);
