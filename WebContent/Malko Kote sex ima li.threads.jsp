...
<title><%=sf.geSubtForum(3).getSubforum_name() %></title>
</head>
<body>
	
	<% List<Thread> threads = sf.getSuborumThreads(3); %>
	<header class="banner">
            Rumblr  ---
            <%=sf.geSubtForum(3).getSubforum_name() %>
	...
	<form class="transparent" action="CreateSubforumMessage" method="post">
		...
		<input type="hidden" name="viewid" value="<%=sf.geSubtForum(3).getSubforum_name() %>.threads.jsp">
	...
	<form class="thrform transparent" action="CreateSubforumThread" method="post">
		...
		<input type="hidden" name="viewid" value="<%=sf.geSubtForum(3).getSubforum_name() %>.threads.jsp">
		<input type="hidden" name="subforum_id" value="<%=sf.geSubtForum(3).getSubforum_id() %>">
