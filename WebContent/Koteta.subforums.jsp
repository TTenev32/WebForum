...
<title><%=f.getForum(2).getForum_name() %></title>
</head>
<body>
	
	<header class="banner">
            Rumblr   ---
            <%=f.getForum(2).getForum_name() %>
	...
	<% List<Subforum> subforums = f.getForumSubforums(2); %>
	...
	<% List<Thread> threads = f.getForumThreads(2); %>
	...
	<form class="transparent" action="CreateMessage" method="post">
		...
		<input type="hidden" name="viewid" value="Koteta.subforums.jsp">
	...
	<%} %>
		...
		<input type="hidden" name="viewid" value="Koteta.subforums.jsp">
		<input type="hidden" name="subforum_id" value="<%=f.getForum(2).getForum_id() %>">
