...
<title><%=f.getForum(3).getForum_name() %></title>
</head>
<body>

	<header class="banner">
            Rumblr   ---
            <%=f.getForum(3).getForum_name() %>
	...
	<% List<Subforum> subforums = f.getForumSubforums(3); %>
	...
	<% List<Thread> threads = f.getForumThreads(3); %>
	...
	<form class="transparent" action="CreateMessage" method="post">
		...
		<input type="hidden" name="viewid" value="Hishtni panteri.subforums.jsp">
	...
	<%} %>
		...
		<input type="hidden" name="viewid" value="Hishtni panteri.subforums.jsp">
		<input type="hidden" name="subforum_id" value="<%=f.getForum(3).getForum_id() %>">
